...
namespace priv {
template < typename T >
T compute_g0( const disk::point< T, 2 > &pt, const static_vector< T, 2 > &n ) {
...
template < typename T >
T compute_g0( const disk::point< T, 3 > &pt, const static_vector< T, 3 > &n ) {
...
T compute_gap_fb( const Mesh &msh, const Elem &elem, const ElemBasis &eb,
                  const disk::dynamic_vector< T > &tab_coeff, const disk::point< T, 2 > &pt,
...
template < typename Mesh, typename Elem, typename ElemBasis, typename T >
T compute_gap_fb( const Mesh &msh, const Elem &elem, const ElemBasis &eb,
                  const disk::dynamic_vector< T > &tab_coeff, const disk::point< T, 3 > &pt,
...
class contact_contribution {
  private:
    ...
    scalar_type make_hho_distance( const point_type &pt, const vector_static &n ) const {
    ...
    vector_type make_hho_phi_n_uT( const vector_type &sigma_nn, const vector_type &uT_n,
                                   const scalar_type &theta, const scalar_type &gamma_F ) const {
    ...
    vector_type make_hho_phi_n_uF( const vector_type &sigma_nn, const vector_type &uF_n,
                                   const scalar_type &theta, const scalar_type &gamma_F,
    ...
    vector_static make_proj_alpha( const vector_static &x, const scalar_type &alpha ) const {
    ...
    matrix_static make_d_proj_alpha( const vector_static &x, const scalar_type &alpha ) const {
        const scalar_type x_norm = x.norm();
                ...
                for ( auto &qp : qps ) {
                    ...
                    if ( contact_type == disk::SIGNORINI_CELL ) {
                        ...
                        const scalar_type phi_n_1_u =
                            eval_phi_n_uT( ET_uTF, gb, cb, uTF, n, gamma_F, qp.point() );
  ...
  public:
    ...
    scalar_type eval_phi_n_uT( const vector_type &ET_uTF, const GradBasis &gb, const CellBasis &cb,
                               const vector_type &uTF, const vector_static &n,
                               const scalar_type &gamma_F, const point_type &pt ) const {
    ...
    template < typename GradBasis, typename CellBasis >
    scalar_type eval_proj_phi_n_uT( const vector_type &ET_uTF, const GradBasis &gb,
                                    const CellBasis &cb, const vector_type &uTF,
                                    const vector_static &n, const scalar_type &gamma_F,
                                    const point_type &pt ) const {
    ...
    vector_static eval_proj_phi_t_uT( const vector_type &ET_uTF, const GradBasis &gb,
                                      const CellBasis &cb, const vector_type &uTF,
                                      const vector_static &n, const scalar_type &gamma_F,
                                      const scalar_type &s, const point_type &pt ) const {
    ...
    template < typename GradBasis, typename FaceBasis >
    scalar_type eval_proj_phi_n_uF( const face_type &fc, const vector_type &ET_uTF,
                                    const GradBasis &gb, const FaceBasis &fb,
                                    const vector_type &uTF, const size_t offset,
                                    const vector_static &n, const scalar_type &gamma_F,
                                    const point_type &pt ) const {
    ...
    template < typename GradBasis, typename FaceBasis >
    vector_static eval_proj_tresca_phi_t_uF( const vector_type &ET_uTF, const GradBasis &gb,
                                             const FaceBasis &fb, const vector_type &uTF,
                                             const size_t &offset, const vector_static &n,
                                             const scalar_type &gamma_F, const scalar_type &s,
                                             const point_type &pt ) const {
    ...
    vector_static eval_proj_coulomb_phi_t_uF( const face_type &fc, const vector_type &ET_uTF,
                                              ...
                                              const vector_type &uTF, const size_t &offset,
                                              const vector_static &n, const scalar_type &gamma_F,
                                              const scalar_type &Fc, const point_type &pt ) const {
        const vector_static phi_t_1_u =
            eval_phi_t_uF( ET_uTF, gb, fb, uTF, offset, n, gamma_F, pt );
        const scalar_type phi_n_1_u =
