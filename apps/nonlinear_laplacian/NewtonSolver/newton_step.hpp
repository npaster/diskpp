...
class NewtonRaphson_step_laplacian
{
    ...
    typedef disk::gradient_reconstruction_elas_full<  mesh_type, cell_basis_type, cell_quadrature_type,
                                                ...
                                                matrix_basis_type, matrix_quadrature_type>                      grad_type;
...
public:
    NewtonRaphson_step_laplacian(const mesh_type& msh, size_t degree, int l = 0)
...
//             disk::test_gradient_reconstruction<mesh_type, cell_basis_type, cell_quadrature_type,
//             face_basis_type, face_quadrature_type, BoundaryConditionFunction>
//             (m_msh, cl, bcf, gradrec.oper, m_degree);
            ...
            auto elem = disk::compute_elem<matrix_basis_type, matrix_quadrature_type, mesh_type>(m_msh, cl, gtu, m_degree);
            ...
            dynamic_matrix<scalar_type> lhs = disk::assemble_lhs<scalar_type>(gradrec.oper, stab.data, elem.first, beta);

            dynamic_vector<scalar_type> rhs = - disk::assemble_rhs<scalar_type>(gradrec.oper, stu, elem.second, rhs_ext, beta);
...
//          std::cout << m_system_matrix << std::endl;
    ...
    assemble(const LoadFunction& lf, const BoundaryConditionFunction& bcf, const std::vector<matrix_dynamic>& offline_data)
    {
       ...
       for (auto& cl : m_msh)
       {
          matrix_dynamic gradrec_oper = offline_data.at(i);

          tc.tic();
          stab.compute(m_msh, cl, gradrec_oper);
          tc.toc();
          ai.time_stab += tc.to_double();
          ...
          /////// NON LINEAIRE /////////
          tc.tic();
          auto gtu = gradrec_oper * m_solution_data.at(i);
          auto stu = stab.data * m_solution_data.at(i);
          ...
          dynamic_matrix<scalar_type> lhs = disk::assemble_lhs<scalar_type>(gradrec_oper, stab.data, elem.first, beta);

          dynamic_vector<scalar_type> rhs = - disk::assemble_rhs<scalar_type>(gradrec_oper, stu, elem.second, rhs_ext, beta);
