...
#include "NL_laplacian_solver.hpp"
...
void
run_NL_laplacian_solver(const Mesh<T, 2, Storage>& msh, run_params& rp)
{
...
//    };
   ...
   NL_laplacian_solver<Mesh, T, 2, Storage,  point<T, 2> > nl(msh, rp.degree);
...
template<template<typename, size_t, typename> class Mesh,
         typename T, typename Storage>
void
run_NL_laplacian_solver(const Mesh<T, 3, Storage>& msh, run_params& rp)
{
   ...
   NL_laplacian_solver<Mesh, T, 3, Storage,  point<T, 3> > nl(msh, rp.degree);
   nl.verbose(rp.verbose);
...
int main(int argc, char **argv)
{
    ...
    if (std::regex_match(mesh_filename, std::regex(".*\\.typ1$") ))
    {
        ...
        run_NL_laplacian_solver(msh, rp);
        return 0;
    }
