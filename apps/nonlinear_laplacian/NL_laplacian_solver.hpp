...
public:
   NL_laplacian_solver(const mesh_type& msh, size_t degree, int l = 0)
   ...
   compute_offline()
   {
      disk::gradient_reconstruction_elas_full< mesh_type,
                                          cell_basis_type, cell_quadrature_type,
                                          face_basis_type, face_quadrature_type,
                                          matrix_basis_type, matrix_quadrature_type> gradrec(m_degree);
...
//      }
      ...
      size_t n_time_step = 1)
   {
      ...
      NewtonRaphson_solver_laplacian<Mesh<T,DIM,Storage>> newton_solver(m_msh, m_degree, 0);
