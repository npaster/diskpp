...
void
run_CH_mixte_solver(const Mesh<T, 2, Storage>& msh, run_params& rp)
{
...
//    };
   ...
   CH_mixte_solver<Mesh, T, 2, Storage,  point<T, 2> > nl(msh, rp.degree);
...
template<template<typename, size_t, typename> class Mesh,
         typename T, typename Storage>
void
run_CH_mixte_solver(const Mesh<T, 3, Storage>& msh, run_params& rp)
{
   ...
   CH_mixte_solver<Mesh, T, 3, Storage,  point<T, 3> > nl(msh, rp.degree);
   nl.verbose(rp.verbose);
