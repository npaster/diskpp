...
class hyperelasticity_solver
{
 ...
 public:
   hyperelasticity_solver(const mesh_type&                 msh,
                          const param_type&                rp,
     ...
     , m_rp(rp)
   {
      ...
      if (((face_degree - 1) > cell_degree) or (cell_degree > (face_degree + 1)) or
          (cell_degree < 0)) {
         std::cout << "face degree: " << face_degree << std::endl;
         std::cout << "cell degree: " << cell_degree << std::endl;
         std::cout << "'cell_degree' should be 'face_degree + 1' => 'cell_degree' => "
                      "'face_degree -1'. Reverting to 'face_degree'."
                   << std::endl;
   ...
   void compute_initial_state()
   {
      ...
      total_dof_depl_static           = (m_msh.faces_size() - nb_faces_dirichlet) * num_face_dofs;
      ...
      if (m_verbose) {
         ...
         std::cout << "** Numbers of dofs: " << total_dof_depl_static << std::endl;
      }
                      ...
                      const NeumannFunction&           g)
   {
      ...
      NewtonRaphson_solver_hyperelasticity<bqdata_type> newton_solver(
        m_msh, m_bqd, m_rp, m_elas_param, m_boundary_condition);
      ...
      while (!list_step.empty()) {
         ...
         const NewtonSolverInfo newton_info =
           newton_solver.compute(rlf, rbcf, rg, m_gradient_precomputed);
         ...
         if (!m_convergence) {
            ...
            if (time_saving) {
               if (m_rp.m_time_save.front() < old_time + 1E-5) {
                  ...
                  std::string name = "result_k" + std::to_string(m_rp.m_cell_degree) + "_l" +
                                     std::to_string(m_rp.m_face_degree) + "_g" +
                                     std::to_string(m_rp.m_grad_degree) + "_t" +
                                     std::to_string(old_time) + "_";
                  ...
                  //                   try {
                  //                      this->compute_discontinuous_Prr(name +"Prr.msh",
                  //                      "Prr");
                  //                   }
                  //                   catch(const std::invalid_argument& ia){
                  //                      std::cerr << "Invalid argument: " << ia.what()  << "
                  //                      in Prr_disc" << std::endl;
                  //                   }
                  //                   try {
                  //                      this->compute_discontinuous_Prr(name +"Poo.msh",
                  //                      "Poo");
                  //                   }
                  //                   catch(const std::invalid_argument& ia){
                  //                      std::cerr << "Invalid argument: " << ia.what() << "
            ...
            tab_traction.push_back(
              {current_time, this->compute_traction_Pk(44)}); // 21 cyl /3 cav / 44 bloc
         }
      }
   ...
   scalar_type compute_l2_error_displacement(const AnalyticalSolution& as) const
   {
   ...
   template<typename AnalyticalSolution>
   scalar_type compute_l2_error_gradient(const AnalyticalSolution& grad) const
   {
      scalar_type err_dof = scalar_type{0.0};
   ...
   template<typename AnalyticalSolution>
   scalar_type compute_l2_error_energy(const AnalyticalSolution& grad) const
   {

      gradrec_type   gradrec(m_bqd);
      projector_type projk(m_bqd);

      const NeoHookeanLaw<scalar_type> law(
        m_elas_param.mu, m_elas_param.lambda, m_elas_param.type_law);
   ...
   template<typename AnalyticalSolution>
   scalar_type compute_l2_error_PK1(const AnalyticalSolution& grad) const
   {
      gradrec_type   gradrec(m_bqd);
      projector_type projk(m_bqd);
   ...
   template<typename AnalyticalSolution>
   void compute_l2_error_displacement_GP(const std::string&        filename,
                                         const AnalyticalSolution& as) const
   {
      ...
      for (auto& cl : m_msh) {
         ...
         for (auto& qp : qps) {
            ...
            for (size_t i = 0; i < m_bqd.cell_basis.range(0, m_bqd.m_cell_degree()).size();
                 i += DIM) {
            ...
            const std::vector<double> value(
              1,
              std::sqrt(disk::mm_prod(depl - true_depl,
                                      depl - true_depl))); // save the solution at gauss point
   ...
   void compute_discontinuous_displacement(const std::string& filename) const
   {
   ...
   void compute_continuous_displacement(const std::string& filename) const
   {
      const size_t cell_degree   = m_bqd.cell_degree();
      const size_t num_cell_dofs = (m_bqd.cell_basis.range(0, cell_degree)).size();
      ...
      std::vector<std::pair<size_t, std::vector<scalar_type>>> value(nb_nodes,
   ...
   void compute_deformed_CONT(const std::string& filename) const
   {
      const size_t num_cell_dofs = disk::howmany_dofs(m_bqd.cell_basis);
      ...
      for (auto itor = m_msh.points_begin(); itor != m_msh.points_end(); itor++) {
         ...
         for (size_t j = 0; j < DIM; j++)
            coor[j] += value[i_node].second[j] / double(value[i_node].first);

         i_node++;
         const visu::Node tmp_node(coor, i_node, 0);
         gmsh.addNode(tmp_node);
      }
   ...
   void compute_deformed(const std::string& filename) const
   {
      const size_t cell_degree   = m_bqd.cell_degree();
      const size_t num_cell_dofs = (m_bqd.cell_basis.range(0, cell_degree)).size();
   ...
   void compute_discontinuous_PK1(const std::string& filename) const
   {
      visu::Gmesh gmsh(DIM);
      auto        storage = m_msh.backend_storage();
      ...
      const NeoHookeanLaw<scalar_type> law(
        m_elas_param.mu, m_elas_param.lambda, m_elas_param.type_law);
      // const CavitationLaw<scalar_type>  law(m_elas_param.mu, m_elas_param.lambda,
      // m_elas_param.type_law);
   ...
   void compute_PK1_GP(const std::string& filename, const std::string compo = "Prr") const
   {
      visu::Gmesh gmsh    = visu::convertMesh(m_msh);
      auto        storage = m_msh.backend_storage();
      ...
      const NeoHookeanLaw<scalar_type> law(
        m_elas_param.mu, m_elas_param.lambda, m_elas_param.type_law);
      // const CavitationLaw<scalar_type>  law(m_elas_param.mu,
      // m_elas_param.lambda, m_elas_param.type_law);
      ...
      for (auto& cl : m_msh) {
         ...
         for (auto& qp : qps) {
            ...
            const std::vector<double> value(1, val);
   ...
   void compute_discontinuous_VMIS(const std::string& filename) const
   {
      visu::Gmesh gmsh(DIM);
      auto        storage = m_msh.backend_storage();
      ...
      for (auto& cl : m_msh) {
         ...
         for (size_t i = 0; i < cell_nodes.size(); i++) {
            ...
            vm = sigma(0, 0) * sigma(0, 0) + sigma(1, 1) * sigma(1, 1) - sigma(0, 0) * sigma(1, 1) +
                 3 * sigma(0, 1) * sigma(0, 1);

            if (DIM == 3) {
               vm +=
                 sigma(2, 2) * sigma(2, 2) - sigma(0, 0) * sigma(2, 2) - sigma(1, 1) * sigma(2, 2);
   ...
   void compute_continuous_VMIS(const std::string& filename) const
   {
      ...
      for (auto& cl : m_msh) {
         ...
         {
            const NeoHookeanLaw<scalar_type> law(
              elas_param.mu, elas_param.lambda, elas_param.type_law);
         ...
         const auto proj_Cauchy_coeff = proj.compute_cell_grad(msh, cl, sigma_computed);
   ...
   void compute_VMIS_GP(const std::string& filename) const
   {
      visu::Gmesh gmsh    = visu::convertMesh(m_msh);
      auto        storage = m_msh.backend_storage();
   ...
   template<typename AnalyticalSolution>
   void compute_l2error_VMIS_GP(const std::string& filename, const AnalyticalSolution& grad) const
   {
      visu::Gmesh gmsh    = visu::convertMesh(m_msh);
      auto        storage = m_msh.backend_storage();
      ...
      for (auto& cl : m_msh) {
         ...
         for (auto& qp : qps) {
            ...
            if (DIM == 3) {
               vm_true += sigma_true(2, 2) * sigma_true(2, 2) -
                          sigma_true(0, 0) * sigma_true(2, 2) - sigma_true(1, 1) * sigma_true(2, 2);
               vm_true +=
                 3 * (sigma_true(1, 2) * sigma_true(1, 2) + sigma_true(0, 2) * sigma_true(0, 2));
            }
   ...
   template<typename AnalyticalSolution>
   void plot_analytical_VMIS(const std::string& filename, const AnalyticalSolution& grad) const
   {
      visu::Gmesh gmsh    = visu::convertMesh(m_msh);
      auto        storage = m_msh.backend_storage();
   ...
   void compute_discontinuous_J(const std::string& filename) const
   {
      visu::Gmesh gmsh(DIM);
      auto        storage = m_msh.backend_storage();
   ...
   void compute_continuous_J(const std::string& filename) const
   {
      visu::Gmesh gmsh    = visu::convertMesh(m_msh);
      auto        storage = m_msh.backend_storage();
   ...
   void compute_J_GP(const std::string& filename) const
   {
      visu::Gmesh gmsh    = visu::convertMesh(m_msh);
      auto        storage = m_msh.backend_storage();
   ...
   template<typename AnalyticalSolution>
   void compute_l2error_J_GP(const std::string& filename, const AnalyticalSolution& grad) const
   {
      visu::Gmesh gmsh    = visu::convertMesh(m_msh);
      auto        storage = m_msh.backend_storage();
   ...
   template<typename AnalyticalSolution>
   void plot_analytical_J(const std::string& filename, const AnalyticalSolution& grad) const
   {
      visu::Gmesh gmsh    = visu::convertMesh(m_msh);
      auto        storage = m_msh.backend_storage();
   ...
   void compute_discontinuous_Prr(const std::string& filename,
                                  const std::string  compo = "Prr") const
   {
      visu::Gmesh gmsh(DIM);
      auto        storage = m_msh.backend_storage();
   ...
   scalar_type compute_traction_RT(const size_t& id) const
   {
      ...
      for (auto& cl : m_msh) {
         ...
         for (auto fc : fcs) {
            if (m_msh.is_boundary(fc)) {
               ...
               if (b_id == id) {
                  traction.proj_grad_space(m_msh,
                                           cl,
                                           m_gradient_precomputed[cell_i],
                                           m_solution_data[cell_i],
                                           m_elas_param);
   ...
   scalar_type compute_traction_Pk(const size_t& id) const
   {
      Hyperelasticity::discrete_traction_bq<bqdata_type> traction(m_bqd);
      gradrec_type                                       gradrec(m_bqd);
      size_t                                             cell_i = 0;
      ...
      for (auto& cl : m_msh) {
         ...
         for (size_t face_i = 0; face_i < fcs.size(); face_i++) {
            ...
            if (m_msh.is_boundary(fc)) {
               ...
               if (b_id == id) {
                  traction.proj_grad_space(m_msh, cl, GT, m_solution_data[cell_i], m_elas_param);
                  ...
                  for (auto& qp : face_quadpoints) {
                     if (m_bqd.cell_degree() == m_bqd.grad_degree()) {
                        const auto T_iqn =
                          traction.eval_Pk_unstable(m_msh, cl, face_i, qp.point(), m_rp.m_beta);
                        const auto n = disk::normal(m_msh, cl, fc);
                        ret += qp.weight() * disk::mm_prod(T_iqn, n);
                     } else {
   ...
   void save_traction(const std::vector<std::array<scalar_type, 2>>& resu,
   ...
   // A supprimer
   std::pair<scalar_type, scalar_type> compute_l2_error_annulus(const std::string& file_error) const
   {
      ...
      for (auto& cl : m_msh) {
         ...
         for (auto& qp : grad_quadpoints) {
            ...
            const scalar_type a =
              (mat(1, ind_R) - mat(1, ind_R - 1)) / (mat(0, ind_R) - mat(0, ind_R - 1));
            ...
            const scalar_type c =
              (mat(2, ind_R) - mat(2, ind_R - 1)) / (mat(0, ind_R) - mat(0, ind_R - 1));
            const scalar_type d      = mat(2, ind_R - 1) - c * mat(0, ind_R - 1);
            const scalar_type dphi_R = c * R + d;
            matrix_dynamic    grad_ref;
            grad_ref.resize(2, 2);
            grad_ref.setConstant(0.0);
   ...
   // A supprimer
   std::array<scalar_type, 3> displacement_node(const size_t num_node) const
   {
      auto                       storage = m_msh.backend_storage();
      ...
      for (auto& cl : m_msh) {
         ...
         for (size_t i = 0; i < cell_nodes.size(); i++) {
            ...
            if (point_ids == num_node) {
               ...
               for (size_t k = 0; k < m_bqd.cell_basis.range(0, m_bqd.cell_degree()).size();
                    k += DIM)
                  for (size_t j = 0; j < DIM; j++)
   ...
   void test_pt(const Solution& sol, const Gradient& grad) const
   {
   ...
   void plot_annulus_nodes()
   {
      gradrec_type                     gradrec(m_bqd);
      const NeoHookeanLaw<scalar_type> law(
        m_elas_param.mu, m_elas_param.lambda, m_elas_param.type_law);
      auto         storage       = m_msh.backend_storage();
      const size_t nb_nodes      = storage->points.size();
      const size_t cell_degree   = m_bqd.cell_degree();
      const size_t num_cell_dofs = (m_bqd.cell_basis.range(0, cell_degree)).size();

      std::vector<std::pair<size_t, scalar_type>> ur(nb_nodes, std::make_pair(0, 0));
      std::vector<std::pair<size_t, scalar_type>> Prr(nb_nodes, std::make_pair(0, 0));
      std::vector<std::pair<size_t, scalar_type>> Poo(nb_nodes, std::make_pair(0, 0));
      ...
      for (auto& cl : m_msh) {
         ...
         for (size_t i = 0; i < cell_nodes.size(); i++) {
            ...
            const auto urr = depl.dot(er);

            // Add displacement at node
            ur[point_ids].first++;
            ur[point_ids].second += urr;

            Prr[point_ids].first++;
            Prr[point_ids].second += PK1rr;

            Poo[point_ids].first++;
            Poo[point_ids].second += PK1oo;
         }
         cell_i++;
      }

      std::ofstream output;
      output.open("ur_nodes.dat", std::ofstream::out | std::ofstream::trunc);
      ...
      output2.open("Prr_nodes.dat", std::ofstream::out | std::ofstream::trunc);
      ...
      output3.open("Poo_nodes.dat", std::ofstream::out | std::ofstream::trunc);
      ...
      for (size_t i_node = 0; i_node < nb_nodes; i_node++) {
         const auto pt = storage->points[i_node];
         if (pt.x() > 0 && pt.y() > 0) {
            const auto R = std::sqrt(pt.x() * pt.x() + pt.y() * pt.y());
            output << R << "\t" << ur[i_node].second / double(ur[i_node].first) << std::endl;
            output2 << R << "\t" << Prr[i_node].second / double(Prr[i_node].first) << std::endl;
            output3 << R << "\t" << Poo[i_node].second / double(Poo[i_node].first) << std::endl;
         }
      }
      output.close();
      output2.close();
   ...
   void plot_annulus_inco_nodes()
   {
      ...
      std::ofstream output;
      output.open("ur_inco_nodes.dat", std::ofstream::out | std::ofstream::trunc);

      if (!output.is_open()) {
         std::cerr << "Unable to open file "
                   << "ur_inco_nodes.dat" << std::endl;
      }

      std::ofstream output2;
      output2.open("Prr_inco_nodes.dat", std::ofstream::out | std::ofstream::trunc);

      if (!output2.is_open()) {
         std::cerr << "Unable to open file "
                   << "Prr_inco_nodes.dat" << std::endl;
      }

      std::ofstream output3;
      output3.open("Poo_inco_nodes.dat", std::ofstream::out | std::ofstream::trunc);
   ...
   void plot_annulus()
   {
      ...
      std::ofstream output2;
      output2.open("Prr_bar.dat", std::ofstream::out | std::ofstream::trunc);
      ...
      std::ofstream output3;
      output3.open("Poo_bar.dat", std::ofstream::out | std::ofstream::trunc);
      ...
      for (auto& cl : m_msh) {
         ...
         for (auto& qp : qps) {
            if (qp.point().x() > 0 && qp.point().y() > 0) {
               ...
               // Compute displacement
               static_vector<scalar_type, DIM> depl = static_vector<scalar_type, DIM>::Zero();
               for (size_t i = 0; i < m_bqd.cell_basis.range(0, m_bqd.cell_degree()).size();
                    i += DIM) {
                  for (size_t j = 0; j < DIM; j++) {
                     depl(j) += phi.at(i + j)(j) * x(i + j);
                  }
   ...
   void plot_annulus_bary()
   {
      ...
      size_t cell_i = 0;
      for (auto& cl : m_msh) {
          const auto bar = barycenter(m_msh,cl);
          if (bar.x() > 0 && bar.y() > 0) {
             matrix_dynamic GT;
             if (m_rp.m_precomputation) {
                GT = m_gradient_precomputed[cell_i];
             ...
             const auto R = std::sqrt(bar.x() * bar.x() + bar.y() * bar.y());
              output2 << R << "\t" << val1 / grad_quadpoints.size() << std::endl;
             output3 << R << "\t" << val2 / grad_quadpoints.size() << std::endl;
         }
         cell_i++;
      }
