...
namespace Hyperelasticity {
   ...
   public:
              ...
              const vector_type& uTF, const ElasticityParameters elas_param)
      {
         ...
         NeoHookeanLaw<scalar_type>  law(elas_param.mu, elas_param.lambda, 1);
         ...
         for (auto& qp : grad_quadpoints)
         {
            ...
            for(std::size_t i = 0; i < num_grad_dofs; i++) {
               for(std::size_t j = 0; j < num_grad_dofs; j++) {
                  AT(i,j) += qp.weight() * disk::mm_prod(gphi.at(i), tm_prod(tensor_behavior.second, gphi.at(j)));
               } // for j
               aT(i) += qp.weight() * disk::mm_prod(tensor_behavior.first, gphi.at(i));
            } // for i
            
            for(std::size_t i = 0; i < num_cell_dofs; i++) {
               RTF(i) += qp.weight() * disk::mm_prod(load(qp.point()) , c_phi.at(i));
            } // for i
         }
