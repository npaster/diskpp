...
error_type
run_hyperelasticity_solver(const Mesh<T, 3, Storage>& msh, const ParamRun<T>& rp, const ElasticityParameters& elas_param, const size_t& n=0)
{
   ...
   auto solution = [alpha](const point<T,3>& p) -> result_type {
      T fx = 0.0;
      T fy = 0.0;
      T fz = 0.0;
   ...
   auto neumann = [alpha](const point<T,3>& p) -> result_type {
      T fx = 0.0;
      T fy = 0.0;
      T fz = -alpha;
   ...
   if(nl.test_convergence()){
      ...
      std::string name = "Result_block_k" + std::to_string(rp.m_cell_degree) + "_l" + std::to_string(rp.m_grad_degree)
...
//       std::cout << " "<< std::endl;
      ...
      {
         std::cout << "Guessed mesh format: Netgen format" << std::endl;
         auto msh = disk::load_netgen_3d_mesh<RealType>(mesh_filename);
         run_hyperelasticity_solver(msh, rp, param,  num);
         return 0;
      }
   }
