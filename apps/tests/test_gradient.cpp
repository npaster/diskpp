...
test_grad_vector(const mesh_type& msh, const size_t degree, const T epsilon)
{
   ...
   if(error_data > epsilon)
      std::cout << "ecart gradient: data " << error_data << std::endl;
   
   if(error_oper > epsilon)
      std::cout << "ecart gradient: oper " << error_oper << std::endl;
   
   std::cout << "time grad: " << tgrad << " vs grad opt: " << tgrad_opt << std::endl;
