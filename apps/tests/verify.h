...
inline void verify_impl(bool condition,  const char *file, int line, const char *condition_as_string)
{
    if (!condition)
    {
        if (Eigen::g_test_level > 0)
            std::cerr << "WARNING: ";
        std::cerr << "Test " << " failed in " << file << " (" << line << ")" << std::endl
...
#define VERIFY_GE(a, b) ::verify_impl(a >= b, __FILE__, __LINE__, EI_PP_MAKE_STRING(a >= b))
#define VERIFY_LE(a, b) ::verify_impl(a <= b, __FILE__, __LINE__, EI_PP_MAKE_STRING(a <= b))
