...
void
renumber_boundaries_gv(Mesh& msh)
{
    ...
    for (size_t face_i = 0; face_i < msh.faces_size(); face_i++)
    {
        ...
        if (storage->boundary_info.at(face_i).is_boundary)
        {
            ...
            if (bar.x() > T(7) && bar.x() < T(60))
            {
                if(bar.y() > T(0.39) && bar.y() < T(0.6))
...
void
run_tresca_solver(Mesh<T, 2, Storage>& msh, const ParamRun<T>& rp, const disk::MaterialData<T>& material_data)
{
    ...
    // GV
    renumber_boundaries_gv(msh);

    auto zero = [material_data](const point<T, 2>& p) -> result_type { return result_type{0.0, 0}; };
    auto depl = [material_data](const point<T, 2>& p) -> result_type { return result_type{-0.1, -0.6}; };

    bnd.addContactBC(disk::SIGNORINI_FACE, 1, s);
    bnd.addDirichletBC(disk::CLAMPED, 4, zero);
    bnd.addDirichletBC(disk::DY, 3, zero);
    bnd.addDirichletBC(disk::DY, 2, depl);
    // bnd.addDirichletBC(disk::DX, 6, zero);
    // bnd.addNeumannBC(disk::NEUMANN, 1, neum);
