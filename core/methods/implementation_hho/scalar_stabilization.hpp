                                   ...
                                   const CellDegreeInfo<Mesh>&                           cell_infos)
{
    ...
    for (size_t i = 0; i < fcs.size(); i++)
    {
        ...
        if (fdi.hasUnknowns())
        {
            ...
            // PikF(theta)
            matrix_type face_mass = matrix_type::Zero(fbs, fbs);
            matrix_type trace     = matrix_type::Zero(fbs, dbs);
            ...
            for (auto& qp : qps)
            {
                ...
                face_mass += priv::outer_product(priv::inner_product(qp.weight(), f_phi), f_phi);
                trace += priv::outer_product(priv::inner_product(qp.weight(), f_phi), d_phi);
            }
            ...
            LDLT<matrix_type> piKF;
            piKF.compute(face_mass);

            tr.block(0, offset_diff, fbs, dbs) = trace;

            oper.block(0, offset_diff, fbs, dbs) = piKF.solve(trace);

            data += oper.transpose() * face_mass * oper / hF;
                                   ...
                                   const hho_degree_info&                                di)
{
    ...
    return make_scalar_hho_stabilization_diff(msh, cl, reconstruction, cell_infos);
}
...
make_scalar_hho_stabilization_adjoint(const Mesh&                                           msh,
                                      ...
                                      const CellDegreeInfo<Mesh>&                           cell_infos)
{
    const auto stab = make_scalar_hho_stabilization_diff(msh, cl, reconstruction, cell_infos);
