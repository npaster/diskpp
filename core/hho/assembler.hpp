...
namespace hho{
   ...
   class assembler_substitution_scalar_bq
   {
   ...
   public:
      ...
      assembler_substitution_scalar_bq()                 = delete;

      assembler_substitution_scalar_bq(const mesh_type& msh, const BQData& bqd)
      : m_bqd(bqd)
      {
   ...
   public:
      ...
      assembler_substitution_vector_bq(const mesh_type& msh, const BQData& bqd, const BoundaryConditions& boundary_conditions)
      : m_bqd(bqd)
      {
         m_num_unknowns = howmany_dofs(m_bqd.face_basis) * (msh.faces_size() - boundary_conditions.nb_faces_dirichlet());
         matrix = sparse_matrix_type(m_num_unknowns, m_num_unknowns);
         rhs = vector_type::Zero(m_num_unknowns);
      ...
      template<typename LocalContrib, typename Function>
      void
      assemble_nl(const mesh_type& msh, const cell_type& cl, const LocalContrib& lc,
                  const Function& bc, const BoundaryConditions& boundary_conditions,
               ...
               for (size_t face_j = 0; face_j < fcs.size(); face_j++)
               {
                  ...
                  if(!fcj_is_dirichlet_boundary){
                     switch ( boundary_conditions.dirichlet_boundary_type(face_id)){
                        ...
                        default:
                        {
                           std::cout << "Unknown Dirichlet Conditions: we do nothing (assembly)" << std::endl;
                           break;
                        }
                     }
                  }
   ...
   public:
      ...
      assembler_full_substitution_vector_bq(const mesh_type& msh, const BQData& bqd, const BoundaryConditions& boundary_conditions)
      : m_bqd(bqd)
      {
         const size_t cells_dofs = howmany_dofs(m_bqd.cell_basis) * msh.cells_size();
         const size_t faces_dofs = howmany_dofs(m_bqd.face_basis) * (msh.faces_size() - boundary_conditions.nb_faces_dirichlet());
         m_num_unknowns = cells_dofs + faces_dofs;
         matrix = sparse_matrix_type(m_num_unknowns, m_num_unknowns);
         rhs = vector_type::Zero(m_num_unknowns);
            ...
            if(fc_is_boundary){
               ...
               for (size_t face_j = 0; face_j < fcs.size(); face_j++)
               {
                  ...
                  if(!fcj_is_boundary){
                     ...
                     += lc.first.block(cell_basis_size + face_j * face_basis_size, pos ,face_basis_size, face_basis_size) * sol_F;
                  }
               }
            }
         }
               ...
               for (size_t face_j = 0; face_j < fcs.size(); face_j++)
               {
                  ...
                  if(!fcj_is_dirichlet_boundary){
                     switch ( boundary_conditions.dirichlet_boundary_type(face_id)){
                        ...
                        {
                           rhs_bc.block(cell_basis_size + face_j * face_basis_size, 0, face_basis_size, 1)
                           += lc.first.block(cell_basis_size + face_j * face_basis_size, pos ,face_basis_size, face_basis_size) * (proj_bcf - sol_F[face_id]);
                           break;
                        }
                        case CLAMPED:
                        {
                           rhs_bc.block(face_j * face_basis_size, 0, face_basis_size, 1)
                           -= lc.first.block(cell_basis_size + face_j * face_basis_size, pos ,face_basis_size, face_basis_size) * sol_F[face_id];
                           break;
                        }
                        default:
                        {
