...
namespace disk {
   ...
   size_t
   number_of_neumann_faces(const Mesh& msh, const std::vector<BoundaryType>& boundary_neumann)
   {
   ...
   template<typename Mesh>
   size_t
   number_of_lag_conditions(const Mesh& msh, const std::vector<BoundaryType>& boundary_dirichlet,
...
//                      break;
   ...
   public:
      ...
      void
      impose_boundary_conditions(const mesh_type& msh, const Function& bc, const std::vector<vector_type>& sol_faces,
                                 const std::vector<vector_type>& sol_lagr, const std::vector<BoundaryType>& boundary_neumann)
      {
         size_t fbs = face_basis.size();
         size_t face_i = 0;
         for (auto itor = msh.boundary_faces_begin(); itor != msh.boundary_faces_end(); itor++)
               ...
               for(auto& elem : boundary_dirichlet)
               {
                  if(msh.boundary_id(face_id) == elem.id)
                  {
                     ...
                     switch ( elem.boundary_type ) {
                        case CLAMPED:  break;
                        case DX:
                           for(size_t i = 0; i < fbs/DIM; i++)
                              rhs_f(i*DIM) += rhs_bc(i*DIM);
                        break;
                           //                   case DY:
                           //                      nb_lag_conditions -= DIM - 1;
                           //                      break;
   ...
   public:
      ...
      assembler_nl_vector_bq(const mesh_type& msh, const BQData& bqd, const BoundaryConditions& boundary_conditions)
      : m_bqd(bqd)
      {
         const size_t nb_lag_conditions = boundary_conditions.nb_lag();
         m_num_unknowns = m_bqd.face_basis.size() * msh.faces_size() + nb_lag_conditions * m_bqd.face_basis.size()/msh.dimension;
         matrix = sparse_matrix_type(m_num_unknowns, m_num_unknowns);
         rhs = vector_type::Zero(m_num_unknowns);
      }
   ...
   template<typename BQData>
   class assembler_full_nl_vector_bq
   {
      typedef typename BQData::mesh_type          mesh_type;
      typedef typename mesh_type::scalar_type     scalar_type;
      typedef typename mesh_type::cell            cell_type;
   ...
   public:
      ...
      assembler_full_nl_vector_bq()                 = delete;

      assembler_full_nl_vector_bq(const mesh_type& msh, const BQData& bqd)
      : m_bqd(bqd)
      {
         const size_t cells_unknows = m_bqd.cell_basis.size() * msh.cells_size();
         const size_t faces_unknows = m_bqd.face_basis.size() * (msh.faces_size() + msh.boundary_faces_size());
         m_num_unknowns = cells_unknows + faces_unknows;
         matrix = sparse_matrix_type(m_num_unknowns, m_num_unknowns);
         rhs = vector_type::Zero(m_num_unknowns);
      }

      assembler_full_nl_vector_bq(const mesh_type& msh, const BQData& bqd, const BoundaryConditions& boundary_conditions)
      : m_bqd(bqd)
      {
         const size_t cells_unknows = m_bqd.cell_basis.size() * msh.cells_size();
         const size_t faces_unknows = m_bqd.face_basis.size() * (msh.faces_size() + msh.boundary_faces_size());
         const size_t lag_unknows = boundary_conditions.nb_lag() * m_bqd.face_basis.size()/msh.dimension;
         m_num_unknowns = cells_unknows + faces_unknows + lag_unknows;
         matrix = sparse_matrix_type(m_num_unknowns, m_num_unknowns);
         rhs = vector_type::Zero(m_num_unknowns);
      }
                                 ...
                                 const BoundaryConditions& boundary_conditions)
      {
         ...
         for (auto itor = msh.boundary_faces_begin(); itor != msh.boundary_faces_end(); itor++)
         {
            ...
            if(boundary_conditions.is_boundary_dirichlet(face_i)){
               ...
               const size_t lagr_basis_size = nb_lag * lagr_size;
               const size_t lag_pos = boundary_conditions.begin_lag_conditions_faceI(face_dir);
               const size_t face_offset_lagrange = cells_offset + msh.faces_size()  * face_basis_size + lag_pos * lagr_size;
