...
namespace hho{
   ...
   public:
      ...
      gradient_reconstruction_vector_bq(const BQData& bqd) : m_bqd(bqd)
      {
...
//             }
      ...
      public:
         ...
         gradient_reconstruction_vector_full_bq(const BQData& bqd) : m_bqd(bqd)
         {}
...
public:
   ...
   projector_vector_bq(const BQData& bqd) : m_bqd(bqd)
   {}
...
public:
   ...
   assembler_vector_bq(const mesh_type& msh, const BQData& bqd, const BoundaryConditions& boundary_conditions)
   : m_bqd(bqd)
   {
      const size_t faces_dofs = m_bqd.face_basis.size() * msh.faces_size();
      const size_t lag_dofs = boundary_conditions.nb_lags() * m_bqd.face_basis.size() / msh.dimension;
   ...
   void
   impose_boundary_conditions_nl(const mesh_type& msh, const Function& bc,
...
public:
   ...
   : m_bqd(bqd)
   {
      ...
      const size_t faces_dofs = m_bqd.face_basis.size() * (msh.faces_size() + msh.boundary_faces_size());
      m_num_unknowns = cells_dofs + faces_dofs;
      matrix = sparse_matrix_type(m_num_unknowns, m_num_unknowns);
      rhs = vector_type::Zero(m_num_unknowns);
   }

   assembler_full_vector_bq(const mesh_type& msh, const BQData& bqd, const BoundaryConditions& boundary_conditions)
   : m_bqd(bqd)
   {
      const size_t cells_dofs = m_bqd.cell_basis.size() * msh.cells_size();
      const size_t faces_dofs = m_bqd.face_basis.size() * msh.faces_size();
   ...
   impose_boundary_conditions(const mesh_type& msh, const Function& bc)
   {
      ...
      for (auto itor = msh.boundary_faces_begin(); itor != msh.boundary_faces_end(); itor++)
      {
         ...
         const size_t face_offset_lagrange = cells_offset + (msh.faces_size() + face_i) * face_basis_size;
