...
class elas_like_stabilization_bq
{
 ...
 public:
   ...
   elas_like_stabilization_bq(const BQData& bqd)
     : m_bqd(bqd)
   {
      cell_basis      = cell_basis_type(bqd.cell_degree() + 1);
      cell_quadrature = cell_quad_type(2 * (bqd.cell_degree() + 1));
   ...
   void compute_adjoint(const mesh_type& msh, const cell_type& cl, const matrix_type& stab_data)
   {
      ...
      for (size_t face_i = 0; face_i < num_faces; face_i++) {
         ...
         MG.block(0, face_i * face_basis_size, dsr.total_size(), face_basis_size) +=
           BRF.transpose() * face_mass_matrix;
      }
...
class elas_like_stabilization_PIKF_bq
{
 ...
 public:
   ...
   elas_like_stabilization_PIKF_bq(const BQData& bqd)
     : m_bqd(bqd)
   {}
